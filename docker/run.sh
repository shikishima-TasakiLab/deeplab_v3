...
DOCKER_VOLUME="${DOCKER_VOLUME} -v $(dirname ${RUN_DIR}):/workspace/deeplab_v3:rw"
DOCKER_VOLUME="${DOCKER_VOLUME} -v ${XSOCK}:${XSOCK}:rw"
DOCKER_VOLUME="${DOCKER_VOLUME} -v ${XAUTH}:${XAUTH}:rw"
if [[ ${DATASET_DIR} != "" ]]; then
  DOCKER_VOLUME="${DOCKER_VOLUME} -v ${DATASET_DIR}:/workspace/deeplab_v3/datasets:rw"
fi
...
docker run \
    ...
    -w /workspace/deeplab_v3 \
    ${DOCKER_VOLUME} \
    ${DOCKER_ENV} \
    --name deeplab_v3-${DEVICE_ID} \
    deeplab_v3
